...
impl CacheStats {
    ...
    pub fn update_entries(&self, count: u64) { self.total_entries.store(count, Ordering::Relaxed); }
    pub fn update_memory_usage(&self, bytes: u64) { self.memory_usage_bytes.store(bytes, Ordering::Relaxed); }

    pub fn hits(&self) -> u64 { self.hit_count.load(Ordering::Relaxed) }
    pub fn misses(&self) -> u64 { self.miss_count.load(Ordering::Relaxed) }
    pub fn evictions(&self) -> u64 { self.eviction_count.load(Ordering::Relaxed) }
}
...
impl SimpleHistogram {
    ...
    pub fn to_prometheus(&self, name: &str) -> String {
        ...
        for (boundary, counter) in &self.buckets {
            out.push_str(&format!("{}_bucket{{le=\"{}\"}} {}\n", name, boundary, counter.load(Ordering::Relaxed)));
        }
